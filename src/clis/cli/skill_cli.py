...
def create_skill(name: str, auto: bool, tools: Tuple[str, ...]) -> None:
    ...
    from clis.cli import _create_skill_template, _create_skill_with_llm
    ...
    if auto:
        ...
        _create_skill_with_llm(prompt, config_manager, tools_list)
    else:
        click.echo(f"Creating skill template: {name}")
        _create_skill_template(name, config_manager, tools_list)
